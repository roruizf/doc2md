...
### OCR Quality Metrics Summary — 2026-05-10
Structured OCR quality metadata was added for scanned PDF, mixed PDF, and standalone image OCR outputs. When `ocr_applied: true`, frontmatter can now include confidence metrics, extracted text density, suspicious-character ratio, requested/used OCR language, language fallback status, and machine-readable degraded conditions so external systems can estimate OCR reliability without reading the body. Digital and non-OCR documents keep these fields nullable. The output schema, user manual, capabilities guide, troubleshooting guide, README, and changelog were updated to document the new fields and the Docling confidence-unavailable caveat.
